# scanner/ui/scan_review_dialog.py

import tkinter as tk
from tkinter import ttk, scrolledtext, filedialog, messagebox
import threading
import time # Needed for sleep in metadata scan

# We'll need access to core functions for import and for metadata scanning
from ..core import import_games, fetch_igdb_metadata

# Import the theme utility function
from ..utils.theme_utils import apply_widget_theme_recursive

class ScanReviewDialog(tk.Toplevel):
    def __init__(self, parent, app, scanned_games_data, supported_systems_list):
        super().__init__(parent)
        self.parent = parent
        self.app = app # Reference to the main application instance
        self.scanned_games_to_edit = scanned_games_data # Store the game data
        self.supported_systems = supported_systems_list # Store supported systems

        self.title("Scan Review & Metadata Editor")
        self.geometry("900x650") # Adjust size as needed
        self.transient(parent) # Makes it appear on top of the parent window
        self.grab_set() # Makes it modal - user must interact with it before parent
        self.protocol("WM_DELETE_WINDOW", self._on_closing) # Handle close button

        # Apply dark mode to this Toplevel window and its widgets
        self.configure(bg=self.app.style.lookup('.', 'background'))
        apply_widget_theme_recursive(self, self.app.style.lookup('.', 'background'), self.app.style.lookup('.', 'foreground'), self.app.style.lookup('TEntry', 'fieldbackground'))

        # Configure grid layout for the dialog frame
        dialog_frame = ttk.Frame(self, padding="10")
        dialog_frame.pack(fill=tk.BOTH, expand=True)
        dialog_frame.columnconfigure(0, weight=3) # Treeview column
        dialog_frame.columnconfigure(1, weight=1) # Detail panel column
        dialog_frame.rowconfigure(0, weight=1) # Treeview/Detail row
        dialog_frame.rowconfigure(1, weight=0) # Controls row (buttons at bottom)

        # --- Left Pane: Scanned Games Treeview (with simulated inline editing) ---
        game_columns = ("Title", "System", "Genre", "Year", "Developer", "Publisher", "Status")
        self.metadata_tree = ttk.Treeview(dialog_frame, columns=game_columns, show="headings")

        # Setup column headings
        self.metadata_tree.heading("Title", text="Title", anchor=tk.W)
        self.metadata_tree.heading("System", text="System", anchor=tk.W)
        self.metadata_tree.heading("Genre", text="Genre", anchor=tk.W)
        self.metadata_tree.heading("Year", text="Year", anchor=tk.W)
        self.metadata_tree.heading("Developer", text="Developer", anchor=tk.W)
        self.metadata_tree.heading("Publisher", text="Publisher", anchor=tk.W)
        self.metadata_tree.heading("Status", text="Play Status", anchor=tk.W) # Play Status for 'play_status'

        # Setup column widths
        self.metadata_tree.column("Title", width=180, stretch=tk.YES)
        self.metadata_tree.column("System", width=80, stretch=tk.YES)
        self.metadata_tree.column("Genre", width=80)
        self.metadata_tree.column("Year", width=60)
        self.metadata_tree.column("Developer", width=100)
        self.metadata_tree.column("Publisher", width=100)
        self.metadata_tree.column("Status", width=80)

        self.metadata_tree.grid(row=0, column=0, sticky="nsew", padx=(0, 5))

        # Populate the Treeview with scanned games
        self._populate_metadata_tree(self.scanned_games_to_edit)

        # --- Right Pane: Detail Editor ---
        detail_frame = ttk.LabelFrame(dialog_frame, text="Selected Game Details", padding="10")
        detail_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0))
        detail_frame.columnconfigure(0, weight=1)
        detail_frame.rowconfigure(3, weight=1) # Description text box should expand

        # Title label for detail panel
        self.detail_title_label = ttk.Label(detail_frame, text="Title: (Select a game)", font=('TkDefaultFont', 10, 'bold'))
        self.detail_title_label.grid(row=0, column=0, sticky="ew", pady=(0, 5))

        # Current game's file path (read-only)
        ttk.Label(detail_frame, text="File Path:").grid(row=1, column=0, sticky="w", pady=(5,0))
        self.detail_filepath_label = ttk.Label(detail_frame, text="", wraplength=200, anchor=tk.W)
        self.detail_filepath_label.grid(row=2, column=0, sticky="ew", padx=(0,0))


        ttk.Label(detail_frame, text="Description/Notes:").grid(row=3, column=0, sticky="w", pady=(5,0))
        self.detail_description_text = scrolledtext.ScrolledText(detail_frame, wrap=tk.WORD, height=10, font=('Consolas', 9))
        self.detail_description_text.grid(row=4, column=0, sticky="nsew", pady=(0, 5))

        # Bind selection and double-click for inline editing
        self.metadata_tree.bind("<<TreeviewSelect>>", self._on_metadata_tree_select)
        self.metadata_tree.bind("<Double-1>", self._start_inline_edit)

        # --- Bottom Controls ---
        controls_frame = ttk.Frame(dialog_frame)
        controls_frame.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(10, 0))
        controls_frame.columnconfigure(0, weight=1)

        # Import and Discard Buttons
        self.import_metadata_button = ttk.Button(controls_frame, text="Import Selected Games", command=self._import_selected_games_with_metadata)
        self.import_metadata_button.pack(side=tk.RIGHT, padx=5)

        self.discard_scan_button = ttk.Button(controls_frame, text="Discard Scan", command=self._discard_scan)
        self.discard_scan_button.pack(side=tk.RIGHT, padx=5)

        # --- NEW: Metadata Scan Button ---
        self.scan_metadata_button = ttk.Button(controls_frame, text="Scan Metadata (IGDB)", command=self._start_metadata_scan_thread)
        self.scan_metadata_button.pack(side=tk.LEFT, padx=5)
        # --- END NEW ---


    def _on_closing(self):
        if messagebox.askokcancel("Quit", "Discard scan results and close the editor?", parent=self):
            self.destroy()
            self.parent.grab_release()
            self.app.update_main_status("Scan review discarded.", "info")


    def _populate_metadata_tree(self, games_data):
        for item in self.metadata_tree.get_children():
            self.metadata_tree.delete(item)

        self.metadata_tree_data = {}

        for i, game in enumerate(games_data):
            iid = f"game_{i}"
            self.metadata_tree_data[iid] = game

            # Populate treeview row
            self.metadata_tree.insert('', 'end', iid=iid, values=(
                game.get('title', ''),
                game.get('system', ''),
                game.get('genre', ''),
                game.get('release_year', ''),
                game.get('developer', ''),
                game.get('publisher', ''),
                game.get('play_status', 'Not Played')
            ))
        self.app.log(f"Metadata editor populated with {len(games_data)} games.", "info")


    def _on_metadata_tree_select(self, event):
        selected_iid = self.metadata_tree.focus()
        if selected_iid and selected_iid in self.metadata_tree_data:
            game = self.metadata_tree_data[selected_iid]
            self.detail_title_label.config(text=f"Title: {game.get('title', '')} ({game.get('system', '')})")
            self.detail_filepath_label.config(text=game.get('filepath', ''))
            self.detail_description_text.delete(1.0, tk.END)
            self.detail_description_text.insert(tk.END, str(game.get('description', '')))
            self._current_selected_iid = selected_iid
            self.detail_description_text.bind("<FocusOut>", self._on_description_edit_finish)
            self.detail_description_text.bind("<Return>", lambda e: (self._on_description_edit_finish(e), "break"))

        else:
            self.detail_title_label.config(text="Title: (Select a game)")
            self.detail_filepath_label.config(text="")
            self.detail_description_text.delete(1.0, tk.END)
            self._current_selected_iid = None


    def _on_description_edit_finish(self, event=None):
        if self._current_selected_iid and self._current_selected_iid in self.metadata_tree_data:
            game = self.metadata_tree_data[self._current_selected_iid]
            new_description = self.detail_description_text.get(1.0, tk.END).strip()
            if new_description != game.get('description', ''):
                game['description'] = new_description
                self.app.log(f"Description updated for {game.get('title')}", "info")


    def _start_inline_edit(self, event):
        region = self.metadata_tree.identify("region", event.x, event.y)
        if region != "cell": return

        column = self.metadata_tree.identify_column(event.x)
        column_index = int(column.replace('#', '')) - 1
        item_iid = self.metadata_tree.identify_row(event.y)

        if not item_iid: return

        metadata_keys_map = {
            0: "title",
            1: "system",
            2: "genre",
            3: "release_year",
            4: "developer",
            5: "publisher",
            6: "play_status"
        }

        metadata_key = metadata_keys_map.get(column_index)
        if metadata_key is None: # Use explicit check for None
            return

        game_data_for_item = self.metadata_tree_data[item_iid]
        current_value = str(game_data_for_item.get(metadata_key, ''))

        x, y, width, height = self.metadata_tree.bbox(item_iid, column)
        if x is None or x == '' or width <= 0 or height <= 0:
            return

        if metadata_key == "system":
            editor = ttk.Combobox(self.metadata_tree, textvariable=tk.StringVar(value=current_value),
                                  values=self.supported_systems, state="normal") # Allows free typing and dropdown
        else:
            editor = ttk.Entry(self.metadata_tree)
            editor.insert(0, current_value)

        editor.place(x=x, y=y, width=width, height=height)
        editor.focus_set()

        bg_editor = self.app.style.lookup('TEntry', 'fieldbackground')
        fg_editor = self.app.style.lookup('TEntry', 'foreground')
        editor.config(background=bg_editor, foreground=fg_editor)

        if metadata_key == "system":
            self.app.style.map('TCombobox', selectbackground=[('selected', self.app.style.lookup('TNotebook.Tab', 'background'))])
            self.app.style.map('TCombobox', selectforeground=[('selected', self.app.style.lookup('TNotebook.Tab', 'foreground'))])


        def _on_edit_finish(event_obj=None):
            new_value = editor.get().strip()
            game_data = self.metadata_tree_data[item_iid]
            
            if metadata_key == 'play_status':
                play_status_options = ['Not Played', 'Playing', 'Completed', 'Backlog', 'Abandoned']
                if new_value not in play_status_options:
                    messagebox.showwarning("Invalid Status", f"'{new_value}' is not a valid status. Please use one of: {', '.join(play_status_options)}", parent=self)
                    new_value = game_data.get(metadata_key, 'Not Played')
            
            elif metadata_key == 'release_year':
                try:
                    new_value = int(new_value) if new_value else None
                except ValueError:
                    messagebox.showwarning("Invalid Year", f"'{new_value}' is not a valid year. Must be a number.", parent=self)
                    new_value = game_data.get(metadata_key, None)
            
            if game_data.get(metadata_key) != new_value:
                game_data[metadata_key] = new_value
                self.app.log(f"Updated '{metadata_key}' for '{game_data.get('title')}' to '{new_value}'", "info")

            values = list(self.metadata_tree.item(item_iid, 'values'))
            values[column_index] = new_value
            self.metadata_tree.item(iid, values=values)

            editor.destroy()

        editor.bind("<Return>", _on_edit_finish)
        editor.bind("<FocusOut>", _on_edit_finish)
        if metadata_key == "system":
            editor.bind("<<ComboboxSelected>>", _on_edit_finish)


    def _apply_bulk_edit(self, field_name):
        selected_iids = self.metadata_tree.selection()
        if not selected_iids:
            messagebox.showwarning("No Selection", "Please select games to apply bulk edit to.", parent=self)
            return

        self.app.log("WARNING: _apply_bulk_edit called but bulk controls should be removed.", "warning")
        messagebox.showinfo("Feature Removed", "Bulk edit controls have been removed from this version.", parent=self)


    def _import_selected_games_with_metadata(self):
        from ..core import import_games

        selected_iids = self.metadata_tree.selection()
        if not selected_iids:
            messagebox.showwarning("No Selection", "Please select games to import.", parent=self)
            return

        games_to_import_with_metadata = []
        for iid in selected_iids:
            games_to_import_with_metadata.append(self.metadata_tree_data[iid])

        if not games_to_import_with_metadata:
            self.app.log("No valid games selected for import.", "warning")
            self.app.update_main_status("No games to import.", "warning")
            return

        self.import_metadata_button.config(state=tk.DISABLED)
        self.scan_metadata_button.config(state=tk.DISABLED) # Disable metadata scan during import
        self.discard_scan_button.config(state=tk.DISABLED)
        self.app.log(f"\n--- Importing {len(games_to_import_with_metadata)} games with metadata... ---", "info")
        self.app.update_main_status(f"Importing {len(games_to_import_with_metadata)} games...", "info", duration_ms=0)

        threading.Thread(target=self._run_import_with_metadata,
                         args=(games_to_import_with_metadata, self.app.import_mode.get()),
                         daemon=True).start()

    def _run_import_with_metadata(self, games, import_mode):
        from ..core import import_games

        imported_count = 0
        total_count = len(games)
        try:
            for result in import_games(games, import_mode, lambda msg, tag=None: self.app.log(msg, tag)):
                if result['success']:
                    imported_count += 1
                    iid_to_delete = None
                    for iid, game_data in list(self.metadata_tree_data.items()):
                        if game_data['filepath'] == result['filepath']:
                            iid_to_delete = iid
                            break
                    if iid_to_delete:
                        self.app.master.after(0, self.metadata_tree.delete, iid_to_delete)
                        self.app.master.after(0, lambda id_to_pop=iid_to_delete: self.metadata_tree_data.pop(id_to_pop, None))

            self.app.log(f"--- Import Complete: {imported_count} of {total_count} games added. ---", "info")
            if imported_count == total_count:
                self.app.update_main_status(f"Import Complete: {imported_count} games added.", "success")
            else:
                self.app.update_main_status(f"Import Complete: {imported_count}/{total_count} games added (some failed).", "warning")
        except Exception as e:
            self.app.log(f"Error during metadata import: {e}", "error")
            self.app.update_main_status(f"Metadata Import Failed: {e}", "error")
        finally:
            self.app.master.after(0, self.import_metadata_button.config, {'state': tk.NORMAL})
            self.app.master.after(0, self.scan_metadata_button.config, {'state': tk.NORMAL}) # Re-enable metadata scan
            self.app.master.after(0, self.discard_scan_button.config, {'state': tk.NORMAL})
            self.app.master.after(0, self._check_and_close_dialog_after_import)


    def _check_and_close_dialog_after_import(self):
        if not self.metadata_tree.get_children():
            messagebox.showinfo("Scan Review Complete", "All selected games have been processed.", parent=self)
            self.destroy()
            self.parent.grab_release()

    def _discard_scan(self):
        confirm = messagebox.askyesno("Discard Scan", "Are you sure you want to discard all current scan results?", parent=self)
        if confirm:
            self.app.log("Scan results discarded.", "info")
            self.scanned_games_to_edit = []
            self.metadata_tree_data = {}
            for item in self.metadata_tree.get_children():
                self.metadata_tree.delete(item)
            self.destroy()
            self.parent.grab_release()

    # --- NEW: Metadata Scan Threading and Logic ---
    def _start_metadata_scan_thread(self):
        selected_iids = self.metadata_tree.selection()
        if not selected_iids:
            messagebox.showwarning("No Selection", "Please select games to scan metadata for.", parent=self)
            return
        
        # Get games data for selected items
        games_to_scan_metadata = [self.metadata_tree_data[iid] for iid in selected_iids]

        self.scan_metadata_button.config(state=tk.DISABLED)
        self.import_metadata_button.config(state=tk.DISABLED)
        self.discard_scan_button.config(state=tk.NORMAL) # Discard can stay enabled if they want to discard mid-scan
        self.app.log(f"\n--- Starting metadata scan for {len(games_to_scan_metadata)} games... ---", "info")
        self.app.update_main_status(f"Scanning metadata for {len(games_to_scan_metadata)} games...", "info", duration_ms=0)

        # Disable treeview interactions during scan
        self.metadata_tree.config(selectmode='none') # Prevent selection changes
        # Bind double-click to prevent new editors from opening during scan
        self.metadata_tree.bind("<Double-1>", lambda e: "break") 

        threading.Thread(target=self._run_metadata_scan,
                         args=(games_to_scan_metadata,),
                         daemon=True).start()

    def _run_metadata_scan(self, games_to_scan_metadata):
        # We need IGDB API keys from config.py
        from ..config import IGDB_CLIENT_ID, IGDB_CLIENT_SECRET

        if not IGDB_CLIENT_ID or not IGDB_CLIENT_SECRET:
            self.app.master.after(0, lambda: messagebox.showerror("API Keys Missing", "IGDB Client ID or Secret is missing in config.py. Cannot scan metadata.", parent=self))
            self.app.log("IGDB API keys missing. Metadata scan aborted.", "error")
            self.app.update_main_status("Metadata scan failed: API keys missing.", "error")
            self.app.master.after(0, self._re_enable_metadata_buttons)
            return

        scanned_count = 0
        total_to_scan = len(games_to_scan_metadata)
        
        for i, game_data in enumerate(games_to_scan_metadata):
            # Update status label with current progress
            self.app.master.after(0, lambda msg=f"Scanning metadata for {game_data['title']}... ({i+1}/{total_to_scan})":
                                     self.app.update_main_status(msg, "info", duration_ms=0))
            self.app.log(f"Scanning metadata for '{game_data['title']}' ({game_data['system']})...", "info")
            try:
                # Call the core function to fetch metadata
                fetched_metadata = fetch_igdb_metadata(
                    game_data['title'],
                    game_data['system'],
                    lambda msg, tag=None: self.app.log(msg, tag),
                    IGDB_CLIENT_ID,
                    IGDB_CLIENT_SECRET
                )
                
                if fetched_metadata:
                    # Update game_data with fetched metadata
                    # Only update fields that are not None/empty
                    for key, value in fetched_metadata.items():
                        if value is not None and value != '':
                            game_data[key] = value
                    
                    self.app.log(f"Successfully fetched metadata for '{game_data['title']}'.", "success")
                    scanned_count += 1
                    # Update Treeview display on main thread
                    self.app.master.after(0, lambda gd=game_data: self._update_treeview_row_with_metadata(gd))
                else:
                    self.app.log(f"No metadata found for '{game_data['title']}'.", "warning")
            except Exception as e:
                self.app.log(f"Error scanning metadata for '{game_data['title']}': {e}", "error")
            
            # Add a small delay to avoid hitting API rate limits too quickly
            time.sleep(0.1) # 100ms delay between requests

        self.app.log(f"--- Metadata scan complete: {scanned_count} of {total_to_scan} games updated. ---", "info")
        if scanned_count == total_to_scan:
            self.app.update_main_status(f"Metadata scan complete: {scanned_count} games updated.", "success")
        else:
            self.app.update_main_status(f"Metadata scan complete: {scanned_count}/{total_to_scan} games updated (some failed).", "warning")
        
        self.app.master.after(0, self._re_enable_metadata_buttons)

    def _re_enable_metadata_buttons(self):
        # Re-enable buttons and Treeview interactions
        self.scan_metadata_button.config(state=tk.NORMAL)
        self.import_metadata_button.config(state=tk.NORMAL)
        self.discard_scan_button.config(state=tk.NORMAL)
        self.metadata_tree.config(selectmode='extended') # Re-enable selection
        self.metadata_tree.bind("<Double-1>", self._start_inline_edit) # Re-enable double-click editing

    def _update_treeview_row_with_metadata(self, game_data):
        # Find the iid for the game_data
        for iid, gd in self.metadata_tree_data.items():
            if gd['filepath'] == game_data['filepath']: # Assuming filepath is unique
                # Update the Treeview item's displayed values
                self.metadata_tree.item(iid, values=(
                    game_data.get('title', ''),
                    game_data.get('system', ''),
                    game_data.get('genre', ''),
                    game_data.get('release_year', ''),
                    game_data.get('developer', ''),
                    game_data.get('publisher', ''),
                    game_data.get('play_status', 'Not Played')
                ))
                break